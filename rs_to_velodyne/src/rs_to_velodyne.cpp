...
static int RING_ID_MAP_RUBY[] = { 3,   66,  33,  96,  11,  74,  41,  104, 19,  82,  49,  112, 27,  90,  57,  120,
...
template <typename T_in_p, typename T_out_p>
void add_time(const typename pcl::PointCloud<T_in_p>::Ptr& pc_in, const typename pcl::PointCloud<T_out_p>::Ptr& pc_out)
{
  // to new pointcloud
  int valid_point_id = 0;
  for (int point_id = 0; point_id < pc_in->points.size(); ++point_id)
...
int main(int argc, char** argv)
{
  ...
  private_nh.param<std::string>("frame_id", frame_id, "rslidar");
  private_nh.param<std::string>("sub_tp", sub_tp, "/rslidar_points");
  private_nh.param<std::string>("pub_tp", pub_tp, "/rslidar_velo_points");
  // private_nh.param<std::string>("input_type", input_type, "XYZI");
  // private_nh.param<std::string>("output_type", output_type, "XYZI");

  if (argc < 3)
  {
    if (private_nh.getParam("input_type", input_type) && private_nh.getParam("output_type", output_type))
    {
      ROS_INFO("input_type: %s", input_type.c_str());
      ROS_INFO("output_type: %s", output_type.c_str());
    }
    else
    {
      ROS_ERROR(
  ...
  {
    subRobosensePC = nh.subscribe(sub_tp, 1, rsHandler_XYZI);
  }
  else if (std::strcmp("XYZIRT", input_type.c_str()) == 0)
  {
    subRobosensePC = nh.subscribe(sub_tp, 1, rsHandler_XYZIRT);
  }
  else
  {
    ROS_ERROR("%s", input_type.c_str());
    ROS_ERROR("Unsupported input pointcloud type. Currently only support XYZI and XYZIRT.");
    exit(1);
  }

  pubRobosensePC = nh.advertise<sensor_msgs::PointCloud2>(pub_tp, 1);

  ROS_INFO("Listening to %s, and publishing to %s", sub_tp.c_str(), pub_tp.c_str());
  ros::spin();
  return 0;
}
